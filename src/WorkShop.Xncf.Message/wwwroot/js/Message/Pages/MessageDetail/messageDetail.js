...
new Vue({
    ...
    methods:
    {
        ...
        async getList()
        {
            ...
            await service.get(`/Admin/MessageDetail/Index?handler=MessageDetail&pageIndex=${pageIndex}&pageSize=${pageSize}&keyword=${keyword}&orderField=${orderField}`).then(res => {
        ...
        handleEdit(index, row, flag)
        {
            ...
            // 编辑
            let { id,messageId, userId, isRead } = row;
            that.dialog.data = {
        ...
        updateData()
        {
            ...
            that.$refs['dataForm'].validate(valid => {
                ...
                if (valid)
                {
                    that.dialog.updateLoading = true;
                    let data = {
                        Id: that.dialog.data.id,                        MessageId: that.dialog.data.messageId,
                        UserId: that.dialog.data.userId,
                        IsRead: that.dialog.data.isRead
                    };
                    console.log('add-' + JSON.stringify(data));
                    service.post("/Admin/MessageDetail/Edit?handler=Save", data).then(res => {
                        if (res.data.success)
                        {
                            that.getList();
                            that.$notify({
        ...
        handleDelete(index, row) {
            let that = this
            let ids = [row.id];
            service.post("/Admin/MessageDetail/edit?handler=Delete", ids).then(res => {
                if (res.data.success)
                    {
                        that.getList();
                        that.$notify({
